...
async def recognize(photo: UploadFile = File(...)):
    """Шаг 1: юзер прислал фото — возвращаем предположение ИИ (марка/модель/год)."""
    image_bytes = await photo.read()
    return await recognize_car(image_bytes, photo.content_type or "image/jpeg")
...
async def create_car(
    ...
    """Шаг 2: юзер подтвердил (или поправил) марку/модель — сохраняем в альбом."""
    db_car = Car(owner_id=user.id, **car.model_dump())
    db.add(db_car)
    await db.commit()
...
async def list_cars(
    ...
    """Альбом пользователя с опциональным фильтром по марке/модели."""
    query = select(Car).where(Car.owner_id == user.id)
    if brand:
...
async def delete_car(
    ...
    result = await db.execute(
        select(Car).where(Car.id == car_id, Car.owner_id == user.id)
    )
